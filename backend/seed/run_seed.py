"""
AURA — Complete End-to-End Demo Generator & Seeder
Generates 30 synthetic screenshots directly into demo_data/screenshots
and processes each screenshot through the entire AURA pipeline:
1. File validation & thumbnail generation
2. EasyOCR text extraction
3. Gemini 2.0 Flash vision & metadata extraction
4. AURA Shield sensitivity detection (PUBLIC, PERSONAL, SENSITIVE, CRITICAL)
5. Sentence-Transformers 384-dim semantic embedding
6. Relationship discovery & graph linking
"""
import sys
import os
import asyncio
...
def generate_all_screenshots():
    print(f"Generating 30 demo screenshots to {OUT_DIR}...")
    ...
    make_screenshot("receipt_laptop_amazon.png", (255, 255, 255),
        ...
        texts=[
            ...
            {"style": "box", "text": "ASUS ZenBook 14 OLED — ₹89,990.00", "height": 55, "bg": (255, 252, 235), "color": (20, 20, 20)},
    ...
    make_screenshot("settings_wifi_password.png", (248, 248, 248),
        ...
        texts=[
            ...
            {"style": "box", "text": "Wi-Fi Password: Scryptic@2026#Secure!", "height": 55, "bg": (255, 235, 235), "color": (150, 0, 0), "size": 17},
    ...
    make_screenshot("settings_api_key.png", (248, 248, 248),
        ...
        texts=[
            ...
            {"style": "box", "text": "ghp_Kx9mRTq7YvN3pL2wQeZ1AbC8dFjHuI4oG5sX", "height": 55, "bg": (255, 235, 235), "color": (150, 0, 0), "size": 14},
    ...
    print(f"Generated {len(list(OUT_DIR.glob('*.png')))} screenshots successfully.\n")
...
async def seed_database():
    ...
    from app.services.pipeline import (
        safe_filename, compute_hash, process_memory
    )
    ...
    print(f"Found {len(screenshots)} screenshots in {OUT_DIR}. Processing through AURA pipeline...")
    ...
    async with AsyncSessionLocal() as db:
        for i, img_path in enumerate(screenshots):
            ...
            if existing_mem:
                print(f"  [{i+1}/{len(screenshots)}] Already indexed: {img_path.name} (ID: {existing_mem.id})")
                continue
            ...
            print(f"  [{i+1}/{len(screenshots)}] Processing: {img_path.name}...")
            result = await process_memory(memory_id, str(dest_path), db)
            status = result.get("status", "?")
            rels = result.get("relationships_created", 0)
            sens = result.get("sensitivity_level", "PUBLIC")
            print(f"       Status: {status} | Sensitivity: {sens} | Relationships: {rels}")

    print("\n✅ All 30 screenshots processed and indexed successfully!")
